...
# 0005. Apple 操作の成果物は本リポジトリにプロジェクトスコープで置き、my-claude-code は `scrum-master` の参照専用とする
...
- **依存は一方向。** 本リポジトリが `scrum-master` を利用し、`scrum-master` は
  Apple のアプリを知らない。
- `flow_metrics.py` は vendoring しない。ユーザースコープに導入済みの
  `~/.claude/skills/scrum-master/scripts/flow_metrics.py` を使う。
  コピーを持つと、参照するはずのものから黙って乖離する。
...
**`scrum-master` スキルごと本リポジトリに vendoring する。** 却下。
参照が1リポジトリで完結し、バージョンずれも起きないという利点はあった。
しかし `scrum-master` は Scrum Guide の引用と8本の参照資料を抱えており、
コピーは必ず本家から乖離する。「参照専用」という方針の意味は、
**利用はするが複製はしない**ことである。
...
- 否定：**`flow_metrics.py` への参照が環境依存になる。** `~/.claude/skills/` に
  導入されていることが前提で、未導入なら壊れる。テストは実体があれば検査し、
  無ければ skip として報告する（pass にはしない）。
- 否定：`scrum_block.py` の CSV 列という**リポジトリを跨ぐ契約**が生まれた。
  片側にしか無いものを片側のテストで守ることになる。列名のリテラルを
  本リポジトリのテストに書き、導入済みの実体があれば併せて照合する。
...
- `tests/run-apple-operators.sh` が、(a) 両スキル・両サブエージェントが
  ...
  （プロジェクトスコープで足りることの裏返し）、(c) `scrum-master` を
  vendoring していないこと、(d) パッケージマニフェストとロックファイルが
  無いこと、(e) `CLAUDE.md` が両オペレーターへの委譲と「判断は委譲しない」
  「着手記録のない項目」を明記していることを検査する。
